...
func NewSystem(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.SystemOutboundOptions) (*System, error) {
	return &System{
...
func (s *System) loadSystemProxy() (err error) {
	...
	if sysProxyConf.Automatic.Active && s.isJavascriptEngine() {
		...
		if err != nil {
			s.pacEnable = false
			s.logger.Debug("gpac parser pac file from url error: ", err)
		} else {
			s.pacEnable = true
...
func (s *System) reloadLoop() {
	...
	for {
		...
		case <-ticker.C:
			err := s.loadSystemProxy()
			if err != nil {
				s.logger.Warn("load system proxy error:", err)
				continue
			}
		...
		if outbound, ok := s.outbounds.Load(URL); !ok {
			...
			case C.TypeHTTP:
				outbound, err, _ = s.singleFlight.Do(URL, func() (interface{}, error) {
					...
					http, err := NewHTTP(s.context, s.router, s.logger, httpOption.Tag, httpOption.HTTPOptions)
					if err != nil {
						return nil, err
					}
...
func (s *System) createOutbound(proxyURL string) (adapter.Outbound, error) {
	...
	case "http", "https":
		...
		out, err = NewHTTP(s.context, s.router, s.logger, outOption.Tag, outOption.HTTPOptions)
		if err != nil {
			return nil, err
		}
	case "socks", "socks5":
